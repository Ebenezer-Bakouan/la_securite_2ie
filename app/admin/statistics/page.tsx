...
const Statistics = () => {
  ...
  const [utilisationParJour, setUtilisationParJour] = useState([]);
  const [tauxOccupation, setTauxOccupation] = useState([]);
  const [distributionStatut, setDistributionStatut] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const salles = await sallesResponse.json();
        setSalleData(salles);
        ...
        const demandeStats = [
          { statut: 'en_attente', count: demandes.demandes.filter(d => d.statut_demande === 'en_attente').length },
          { statut: 'approuvee', count: demandes.demandes.filter(d => d.statut_demande === 'approuvee').length },
          { statut: 'rejetee', count: demandes.demandes.filter(d => d.statut_demande === 'rejetee').length }
        ];
        ...
        const userStats = [
          { statut: 'Étudiant', count: users.filter(u => u.statut === 'Étudiant').length },
          { statut: 'Professeur', count: users.filter(u => u.statut === 'Professeur').length },
          { statut: 'Stagiaire', count: users.filter(u => u.statut === 'Stagiaire').length },
          { statut: 'Travailleur 2iE', count: users.filter(u => u.statut === 'Travailleur 2iE').length }
        ];
        ...
        const utilisation = await utilisationResponse.json();
        ...
        const occupation = await occupationResponse.json();
  ...
  const salleOccupationData = salleData.map(salle => ({
    nom: salle.nom,
    pourcentageOccupation: Math.round((salle.nombre_presents / salle.capacite) * 100)
  }));
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md text-center">
          ...
          <p className="text-3xl font-bold text-green-600">
            {salleData.reduce((sum, salle) => sum + salle.capacite, 0)}
          </p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-md text-center">
          <h3 className="font-semibold text-gray-600">Personnes présentes</h3>
          <p className="text-3xl font-bold text-purple-600">
            {salleData.reduce((sum, salle) => sum + salle.nombre_presents, 0)}
          </p>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-md text-center">
          <h3 className="font-semibold text-gray-600">Taux d'occupation moyen</h3>
          <p className="text-3xl font-bold text-orange-600">
            {Math.round(salleData.reduce((sum, salle) => sum + (salle.nombre_presents / salle.capacite), 0) / salleData.length * 100)}%
          </p>
        </div>
      </div>
    </div>
